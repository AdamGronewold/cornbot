...
class OdomTfBroadcaster(Node):
    ...
    def __init__(self):
        ...
        self.left_rpm=0.0
        self.right_rpm=0.0
        self.v_measurement=0.0
        self.omega_measuremnt=0.0
        ...
        self.create_subscription(String, 'gnss/status/fix_quality_topic', self.fix_quality_callback, 1)
        self.create_subscription(PointStamped, 'wheel_rpm_left_right', self.wheel_rpm_callback, 1)
        self.create_subscription(String, 'cornbot/speed_ref_topic', self.speed_ref_callback, 1)
        ...
        self.timer_ekf=self.create_timer(25, self.ekf_callback)
        
    def ekf_callback(self)
    	measurement_vector=[self.current_x, self.current_y, self.current_heading, self.v_measurment, self.omega_measurement]
        
    def wheel_rpm_callback(self, msg)
	self.left_rpm=msg.point.x
	self.right_rpm=msg.point.y
	left_mpers=self.left_rpm*2*math.pi*self.wheel_radius/60 #meters per second
	right_mpers=-1*self.right_rpm*2*math.pi*self.wheel_radius/60
	self.v_measurement=(left_mpers+right_mpers)/2
	self.omega_measurement=left_mpers-right_mpers/self.track
