...
class WaypointNode(Node):
    ...
    def __init__(self):
        ...
        # Robot initial and current positions
        self.lat0 = self.lon0 = None
        self.current_lat = self.current_lon = self.current_heading = self.cur_head_ccw = None
        self.xr = self.yr = None

        # Waypoints
        self.waypoints = [
            {"lat": 43.70409011798969, "lon": -72.29561185773956},
            {"lat": 43.704025196022116, "lon": -72.29592550489402}
            #Loop
        ...
        # Paddle threshold for feeler
        self.paddle_threshold = 25  # Example threshold value

        # Temporary waypoint
        self.temp_waypoint = None
        
        # Create a thread for high-frequency feeler data processing
    ...
    def get_init_heading(self):  # ✓
        ...
        while self.current_heading is None:  # If we don't yet have a reference heading from the GNSS node, initialize by moving forward until we do.
            self.publish_ref_speed(-0.1, -0.1)
            self.get_logger().info('Moving until heading set.')
            rclpy.spin_once(self)    

    def variable_init(self):  # ✓
        # initialize lat, lon
        while self.current_lat is None and self.current_lon is None:  # sit stationary until lat and lon start arriving on the topic
    ...
    def update_position_callback(self, msg):  # ✓
        try:
            ...
            time.sleep(1)
            self.get_logger().info(f'Error updating position. Shutting down node...')
            self.destroy_node()
            rclpy.shutdown() 
    ...
    def update_heading_callback(self, msg):  # ✓
        try:
            ...
            self.publish_ref_speed(0.0, 0.0)
            time.sleep(1)
            self.get_logger().info(f'Error updating heading. Shutting down node...')
            self.destroy_node()
            rclpy.shutdown() 
    ...
    def update_angles_callback(self, msg):  # ✓
        try:
            ...
            # Check for paddle threshold and inject waypoint if necessary
            if self.FL < -self.paddle_threshold:
                self.get_logger().info(f'Injecting waypoint to the right of the robot')
                self.inject_temporary_waypoint('left', self.FL)
            if self.FR > self.paddle_threshold:
                self.get_logger().info(f'Injecting waypoint to the left of the robot')
                self.inject_temporary_waypoint('right', self.FR)
                
        except Exception as e:
            self.publish_ref_speed(0.0, 0.0)
            time.sleep(1)
            self.get_logger().info(f'Error updating feeler angles. Shutting down node...')
            self.destroy_node()
            rclpy.shutdown() 
    ...
    def find_new_waypoint_position(self, direction, angle):
        distance = self.lookahead_distance  # Distance from the robot to the new waypoint

        # Convert the robot's current heading to radians
        heading_rad = math.radians(self.cur_head_ccw)

        # Adjust the driving angle based on the feeler deflection
        K=1/2
        angle_rad = math.radians(angle)*K

        if direction == 'left':
            adjusted_heading = heading_rad + angle_rad
        elif direction == 'right':
            adjusted_heading = heading_rad + angle_rad

        # Calculate the new waypoint coordinates
        new_x = self.xr + distance * math.cos(adjusted_heading)
        new_y = self.yr + distance * math.sin(adjusted_heading)
        self.get_logger().info(f'New waypoint injected at ({new_x}, {new_y})')
        return {"x": new_x, "y": new_y, "timestamp": time.time()}

    def inject_temporary_waypoint(self, direction, angle): # Inject temporary waypoint into the navigation path
        new_waypoint = self.find_new_waypoint_position(direction, angle)

        if self.temp_waypoint:
            # Remove old temporary waypoint
            self.waypoints_local.remove(self.temp_waypoint)

        # Find the new closest segment and the index to inject
        self.find_closest_path_segment()

        # Insert the new temporary waypoint into the waypoints list
        self.waypoints_local.insert(self.closest_segment_index + 1, new_waypoint)

        # Store the new temporary waypoint
        self.temp_waypoint = new_waypoint
    ...
    def plot_navigation(self):
        if plt.fignum_exists(1):
            ...
            for wp in wp_xy:
                ...
                if label is None:
                    label = "Waypoints"
                    self.ax.scatter(wp["x"], wp["y"], color=color, marker=marker, s=size, label=label)
                    self.ax.text(wp["x"], wp["y"], f"({wp['x']:.2f}, {wp['y']:.2f})", fontsize=9, verticalalignment='bottom')
                else:
                    self.ax.scatter(wp["x"], wp["y"], color=color, marker=marker, s=size)
                    self.ax.text(wp["x"], wp["y"], f"({wp['x']:.2f}, {wp['y']:.2f})", fontsize=9, verticalalignment='bottom')
...
#_____________________________________________________________________
    ...
    def update_data_loop(self):
        while rclpy.ok():
            rclpy.spin_once(self, timeout_sec=0.008)  # 125 Hz
    ...
    def pure_pursuit(self):
        ...
        while rclpy.ok():
            ...
            try:
                ...
                self.publish_ref_speed(0.0, 0.0)
                time.sleep(1)
                self.get_logger().info(f'Exception during plot initialization. Shutting down node...')
                self.destroy_node()
                rclpy.shutdown()
            ...
            try:
                ...
                self.publish_ref_speed(0.0, 0.0)
                time.sleep(1)
                self.get_logger().info(f'Exception while initializing variables: {e}. Shutting down node...')
                self.destroy_node()
                rclpy.shutdown()     
            ...
            try:
                ...
                self.publish_ref_speed(0.0, 0.0)
                time.sleep(1)
                self.get_logger().info(f'Exception while initializing heading: {e}. Shutting down node...')
                self.destroy_node()
                rclpy.shutdown() 
            ...
            while not self.reached_goal():  # ✓
                try:
                    ...
                    # Main loop for position and navigation updates
                    #rclpy.spin_once(self, timeout_sec=0.25)  # 4 Hz
                    ...
                    # Remove the temporary waypoint if it has expired
                    if self.temp_waypoint and (time.time() - self.temp_waypoint["timestamp"] > 10):
                        self.waypoints_local.remove(self.temp_waypoint)
                        self.temp_waypoint = None
