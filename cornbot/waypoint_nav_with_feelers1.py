...
class WaypointNode(Node):
    ...
    def __init__(self):
        ...
        self.BL=0
        self.BR=0
        self.paddle_threshold=30 #degrees
        
        # Variable to track the temporary waypoint for obstacle avoidance
    ...
    def update_angles_callback(self, msg):
        ...
        if self.FL < -self.paddle_threshold:
            if self.active_segment:
                # Refresh the waypoint list to only include the original waypoints
                self.waypoints_local = self.waypoints_local_static

                # Remove existing temporary waypoint if present
                if self.temp_waypoint:
                    self.waypoints_local.remove(self.temp_waypoint)
                    self.temp_waypoint = None
                    

                # Inject a temp waypoint
                self.get_logger().info('Left side collision. Injecting waypoint on right side.')
                self.find_new_waypoint_position('right', abs(self.FL))
                self.insert_waypoint(self.new_waypoint)  # Inject the waypoint somewhere to the right of the robot
        ...
        if self.FR > self.paddle_threshold:
            if self.active_segment:
                ...
                # Inject a temp waypoint
                self.get_logger().info('Right side collision. Injecting waypoint on left side.')
                self.find_new_waypoint_position('left', abs(self.FR))
                
                self.insert_waypoint(self.new_waypoint)  # Inject the waypoint somewhere to the left of the robot
 
    def insert_waypoint(self, new_waypoint):
        # Find the index of the closest "test" segment's end point in the local waypoints list
        segment_end = self.test_segment[1]
        print("MAde it here")
        segment_end_index = next(i for i, wp in enumerate(self.waypoints_local) 
                                    if wp["x"] == segment_end[0] and wp["y"] == segment_end[1])
        
        # Insert the new waypoint at the position before the end of the active segment
        self.waypoints_local.insert(segment_end_index, new_waypoint)
        self.temp_waypoint = new_waypoint
        self.get_logger().info(f'Inserted new waypoint at index {segment_end_index}: {new_waypoint}')

        
        
    def find_new_waypoint_position(self, direction, angle):
        # Calculate the new waypoint coordinates based on the current position, heading, and feeler angle
        distance = self.look_ahead_distance * 2  # Distance from the robot to the new waypoint
        ...
        # Add a timestamp to the waypoint
        self.new_waypoint = {"x": new_x, "y": new_y, "timestamp": time.time()}
    ...
    def update_course_callback(self, msg):
        ...
        self.cur_head_ccw = (90 - theta_cw) % 360 #measured counterclockwise from x+ in degrees up to 360
    ...
    def action_init(self):
        ...
        for i in range(len(self.waypoints)):
            ...
            self.waypoints_local_static=self.waypoints_local #waypoints local is a dynamic list that changes during operation based on collisions. Waypoints_local_static is the list of the original waypoints.
    ...
    def find_active_path_segment(self):
        # 1. Initialize variables to find the segment with the closest point, ie. the "test segment"
        ...
        # 2. Iterate through each segment of the path to find which contains the closest point
        for i in range(len(self.waypoints_local) - 1):  # Loop through waypoints, stopping before the last one
            ...
            x1, y1 = wp1["x"], wp1["y"]  # Coordinates of the current waypoint being considered
        ...
        #self.get_logger().info(f"Closest point on path: {self.test_seg_closest_point}, Test segment: {self.test_segment}") <------- This is the test segment or the closest segment comprised of two of the original waypoints
        ...
        while rclpy.ok():
            ...
            while not self.reached_goal():
                try:
                    ...
                    # Monitor and remove temporary waypoint if it has existed for more than 10 seconds
                    if self.temp_waypoint and (time.time() - self.temp_waypoint["timestamp"] > 10):
                        self.waypoints_local.remove(self.temp_waypoint)
                        self.temp_waypoint = None
                        self.get_logger().info('Removed temporary waypoint after 10 seconds')
